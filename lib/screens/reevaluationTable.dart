...
class ReEvaluationTable extends StatefulWidget {
  @override
  _ReEvaluationTableState createState() => _ReEvaluationTableState();
}

class _ReEvaluationTableState extends State<ReEvaluationTable> {
  ...
  Future<void> fetchData() async {
    ...
    setState(() {
      _data = data.where((patient) {
        return patient['data']['reeval'] == "1";
      }).toList();
      print(_data);
      _data.sort((a, b) {
        DateTime dateA = _parseDate(a['data']['followUpDate']);
        DateTime dateB = _parseDate(b['data']['followUpDate']);
  ...
  void _updateEnquiryStatus(String docId, bool add) async {
    await _firestore
        .collection(db)
        .doc(docId)
        ...
        return AlertDialog(
          ...
          actions: <Widget>[
            ...
            TextButton(
              child: Text('Confirm'),
              onPressed: () {
                _updateEnquiryStatus(docId, add);
                Navigator.of(context).pop();
              },
            ),
          ],
